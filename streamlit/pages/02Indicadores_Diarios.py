...
# Redondear hora al bloque de 5 minutos más cercano para que st.cache_data use la misma clave durante 300 segundos
def obtener_bloque_5min():
    ahora = datetime.now()
    minuto_bloque = (ahora.minute // 5) * 5
    ahora_bloque = ahora.replace(minute=minuto_bloque, second=0, microsecond=0)
    return pd.to_timedelta(ahora_bloque.strftime('%H:%M:%S'))
...
# Auto-refresh cada 5 minutos (300 segundos)
...
def render_dashboard():
    ...
    # Determinar hora de referencia estandarizada en bloques de 5 min
    es_hoy = selected_date == datetime.now().date()
    hora_referencia = obtener_bloque_5min() if es_hoy else pd.to_timedelta('22:00:00')

    if st.sidebar.button("Actualizar"):
        st.cache_data.clear()

    df = obtener_df(fecha_actual, hora_referencia)
    ...
    if df.empty:
        ...
        hs_total = (hora_referencia.total_seconds() / 3600) - primera_hora
